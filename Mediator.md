...
##  管道作用：
```
1.Mediator：发送命令至GlobalReceivePipe；
2.GlobalReceivePipe: 每当消息在到达下一个管道和处理程序之前发送、发布或请求时，都会触发此管道；
3.CommandReceivePipe: 接收Icommand接口的命令至处理程序(CommandHandler);我们用于发送增删改命令
4.RequesteReceivePipe: 接收IRequest接口的命令至处理程序(RequestHandler);我们用于发送查询命令
5.EventReceivePipe: 接收IEvent接口的命令至处理程序中(EventHandler);
6.Handler: 处理程序，处理各种命令，以及发布消息；
7.publishPipe: 接收IEvent接口的事件，我们通常用于处理后续事件或命令，没有则结束；
```
##  Mediator配置步骤：
一、引用Mediator.Net与Mediator.Net.Autofac包；  
```
1.Mediator.Net包作用：提供了一种中介者模式的实现；提供中介者模式封装的接口和方法、类，如消息传递、事件处理、命令调度;  
2.Mediator.Net.Autofac的作用:提供依赖注入;
```
...
public static class UnitOfWorkMiddleWare
{
     ...
     public static void UseUnitOfWork<TContext>(this IPipeConfigurator<TContext> configurator, IUnitOfWork unitOfWork = null) where TContext : IContext<IMessage>
     {
        ...
        $"{nameof(unitOfWork)} is not provided and IDependencyScope is not configured,Please ensure {nameof(unitOfWork)} is registered properly if you are using IoCcontainer,
